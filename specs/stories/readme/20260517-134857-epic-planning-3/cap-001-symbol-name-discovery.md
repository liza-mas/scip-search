...
### Acceptance Criteria
- AC-002-1: Given a matching SCIP symbol in the loaded index, when the Automation Agent runs `symbols --name <name>`, then each default one-line result begins with `path:line:column:` and includes `packageKey`, one space, and the exact symbol `descriptor`.
...
- AC-002-3: Given a one-line result, when the Automation Agent concatenates `packageKey`, one space, and `descriptor` from the metadata after the third colon, then the result is the exact full SCIP symbol string from the index.
...
- AC-002-4b: Given traversal does not provide a definition location for a matching symbol, when the Automation Agent reads the returned one-line result, then the result begins with `?:0:0:` and still includes the exact descriptor and available match context without turning the successful match into an error.
...
- AC-002-7: Given a one-line result, when the Automation Agent parses the output, then only the `path:line:column:` prefix is stable colon-delimited location data; metadata after the third colon is grep-style human-readable text.
